...
async def on_reaction_remove(reaction, user):  
    nome = user.name
    username.remove(nome)
    msg = reaction.message
    if reaction.emoji == emoji and msg.id == msg_id: #and user == msg_user:
        role = discord.utils.find(lambda r: r.name == "Avalon", msg.guild.roles)
        await user.remove_roles(role)
        print("remove")
        msg_embed1 = discord.Embed(
        title="Avalon" ,
        description=f"-IP MINIMO DPS: {ip} ARMA E OFHANGE + IP GERAL: {ip - 300}\n"
        f"-IP MINIMO SUPORTES: {ip - 300}\n\n"
        f"SAIDA DE FORT\n"
        f"CLASSES\n"
        f"1X OFFTANK({ip})\n"
